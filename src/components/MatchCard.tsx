...
interface MatchCardProps {
  ...
  awayTeam: Team | null;
  workspaceId: string;
  userBet?: { prediction: string; gems_wagered: number; gems_won: number; resolved: boolean } | null;
}

function TeamDisplay({ team, score, isWinner }: { team: Team | null; score: number | null; isWinner: boolean }) {
  return (
    <div className={`flex items-center gap-2 ${isWinner ? 'font-bold' : ''}`}>
      {team?.crest_url && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={team.crest_url} alt={team.tla || team.name} className="w-6 h-6 object-contain" />
      )}
      <span className="truncate">{team?.tla || team?.name || 'TBD'}</span>
      {score !== null && (
        <span className="ml-auto text-lg font-mono">{score}</span>
      )}
    </div>
  );
}
...
export default function MatchCard({ match, homeTeam, awayTeam, workspaceId, userBet }: MatchCardProps) {
  ...
  return (
    <Link
      ...
      className={`block rounded-lg border p-4 transition-colors hover:bg-card-hover ${
        isLive ? 'border-danger bg-card animate-pulse' : 'border-card bg-card'
      }`}
    >
      <div className="flex items-center justify-between mb-2 text-xs text-silver">
