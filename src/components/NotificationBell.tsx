...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("notifications")
      .select("id, type, title, body, read, gems_delta, created_at")
...
function NotificationItem({ notification: n }: { notification: Notification }) {
  const isPositive = n.gems_delta > 0;
  const isNegative = n.gems_delta < 0;

  return (
    <div className={`px-4 py-3 ${n.read ? "opacity-70" : ""}`}>
